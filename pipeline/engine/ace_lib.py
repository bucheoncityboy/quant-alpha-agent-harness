...
# ── Inlined from helpful_functions.py ──────────────────────────────────


def expand_dict_columns(df):
    """Expand dictionary columns in a DataFrame."""
    return df
...
def save_pnl(pnl_df, alpha_id, region):
    ...
    filename = f"pnl/{region}_{alpha_id}.csv"
    pnl_df.to_csv(filename)
    return filename
...
def save_simulation_result(result):
    ...
    filename = f"results/{alpha_id}_simulation.json"
    with open(filename, "w") as f:
        json.dump(result, f, indent=2)
    return filename


def save_yearly_stats(stats_df, alpha_id, region):
    """Save yearly stats to file."""
    os.makedirs("stats", exist_ok=True)
    filename = f"stats/{region}_{alpha_id}_yearly.csv"
...
def get_credentials() -> Tuple[str, str]:
    ...
    credential_email = os.environ.get("BRAIN_ID")
    credential_password = os.environ.get("BRAIN_PASSWORD")
...
def submit_alpha(s: SingleSession, alpha_id: str) -> dict:
    """
    Submit an alpha to make it ACTIVE on WQ BRAIN.

    Calls POST /alphas/{alpha_id}/submit. After a successful submit,
    the alpha status changes to "SUBMITTED" / "ACTIVE" on the platform.
    ...
    Returns:
        dict: The API response JSON on success, or an error dict with
              ``{"success": False, "status_code": N, "detail": "..."}``
              on failure.
    """
    url = brain_api_url + f"/alphas/{alpha_id}/submit"
    for attempt in range(3):
        try:
            ...
            if r.status_code == 429:
                ...
                logger.warning(
                    "Rate limited (429) submitting alpha %s — "
                    "retrying after %.0fs (attempt %d/3)",
                    alpha_id, retry_after, attempt + 1,
                )
            ...
            if r.text.strip():
                try:
                    ...
                    logger.info("Submitted alpha %s — status=%s",
                                alpha_id, status)
                    return result
                except Exception:
                    pass
            # Accept empty 2xx response as success (WQ API quirk)
            logger.info("Submitted alpha %s (HTTP %d, empty body)", alpha_id, status_code)
            return {"success": True, "status_code": status_code, "alpha_id": alpha_id}
        except requests.exceptions.HTTPError as exc:
            status = exc.response.status_code if exc.response is not None else 0
            detail = exc.response.text[:200] if exc.response is not None else str(exc)
            logger.error("Submit alpha %s failed (HTTP %d): %s",
                         alpha_id, status, detail)
            if status in (400, 422):
                return {"success": False, "status_code": status, "detail": detail}
            if attempt < 2:
                time.sleep(5)
                continue
            return {"success": False, "status_code": status, "detail": detail}
    return {"success": False, "detail": "All retries exhausted"}
