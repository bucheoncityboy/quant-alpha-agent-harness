...
- **유효 알파 70개 발굴** — 플랫폼 기준을 통과한 시그널을 지속 확보 (제출 기록 14건)
- **통계 가드레일 + 5-Gate 제출 파이프라인** — 자격 미달 알파는 제출 경로가 차단되는 Fail-Closed 구조
- **리서치 컨설턴트 계약 체결** — WorldQuant 산하 리서치 컨설턴트와 계약하며 실전 운영
- **NDA-safe 공개**: 개별 알파 수식 원문·데이터셋 카탈로그는 플랫폼 규정에 따라 공개 범위에서 제외. 에이전트 제어 프레임워크만 공개한다.

## 검증 대상 질문

"LLM 에이전트가 실제로 유효한 알파 시그널을 발굴할 수 있는가?"

이 질문에 답하기 위해 무작위 조합(auto-loop)이 아니라 **에이전트가 조합·검토·제출 결정을 내리는** 대화형 마이닝 워크플로우를 구축했다. 검증 대상은 두 가지다.
